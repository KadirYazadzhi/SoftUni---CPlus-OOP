...
public:
    ...
    const MemoryNode &getAt(int index) const {
        return memory[index];
    }
...
public:
    DualParameterCommand(MemoryHelper &memory, istream &istr)
        ...
        if (index < 0 || index >= static_cast<int>(memory.getSize())) {
...
public:
    AllocateCommand(MemoryHelper &memory, istream &istr)
            : DualParameterCommand(memory, istr) {
...
public:
    DeallocateCommand(MemoryHelper &memory, istream &istr)
            : DualParameterCommand(memory, istr) {
        if (getResult() != ErrorCode::EXECUTE_SUCCESS) {
            return;
        }
